...
考虑到现阶段大部分RH中的ExchangeID字段返回的都是空值
vtSymbol直接使用symbol
'''
...
########################################################################传统批
class RhGateway(VtGateway):
    """RH接口"""

    #----------------------------------------------------------------------
    def __init__(self, eventEngine, gatewayName='RH',mode = 'Test'):
        """Constructor"""
        super(RhGateway, self).__init__(eventEngine, gatewayName)
        
        self.mdApi = RhMdApi(self)     # 行情API
        self.tdApi = RhTdApi(self)     # 交易API
    ...
    def connect(self):
        """连接"""
        try:
            with open(self.filePath, 'r') as load_f:
                # 解析json文件
        ...
        try:
            ...
            brokerID = str(setting[self.mode]['brokerID'])
            tdAddress = str(setting[self.mode]['tdAddress'])
            mdAddress = str(setting[self.mode]['mdAddress'])
            
            # 如果json文件提供了验证码
            if 'authCode' in setting: 
                authCode = str(setting[self.mode]['authCode'])
                userProductInfo = str(setting[self.mode]['userProductInfo'])
...
class RhTdApi(rhTdApi):
        ...
        if last:
            ...
            for ix,pos in enumerate(self.posDict.values()):
                if ix <len(self.posDict.values()) - 1:
                    ...
                    islast = True
                self.gateway.onPosition(pos,islast)
    ...
    def onRtnOrder(self, data):
        ...
        order.orderID = data['OrderRef']
        # Rh的报单号一致性维护需要基于frontID, sessionID, orderID三个字段
        # 但在本接口设计中，已经考虑了Rh的OrderRef的自增性，避免重复
